...
class SmallBoard(Board):
    ...
    def winner(self):
        ...
        if self.cells[2].id == self.cells[4].id and self.cells[2].id == self.cells[6].id \
            ...
            self.won = (self.cells[2].id, 0, self.line_diagonal_l)      # the first cell is 0 for drawing purposes
            return
...
class BigBoard(Board):
    ...
    def __init__(self, start_x, start_y, small_boards, board, big_x,
                 big_o, line_horizontal, line_vertical, line_diagonal_l, line_diagonal_r):
